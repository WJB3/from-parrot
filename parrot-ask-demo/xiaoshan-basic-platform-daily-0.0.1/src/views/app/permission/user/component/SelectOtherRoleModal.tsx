...
// type 1: 更新 2：选择
export default function SelectOtherRoleModal(props: GT.Modal.Props & {type: 1 | 2}) {
  const [visible, setVisible] = useState(false);
  const [title, setTitle] = useState('分配角色');
  const [loading, setLoading] = useState(false);
  const [keyword, setKeyword] = useState('');
  const { data: roles } = useRequest(api.user.getOtherRolesOptions);
  ...
  const handleOk = () => {
    form.validateFields().then(() => {
        if (props.type == 1) {
            ...
            setLoading(true);
            api.user
              .updateOtherRoles(data.id, data.roleIds)
              .then(() => {
                setVisible(false);
            ...
            setVisible(false);
            props.onOk && props.onOk(formData?.roleIds);
        }
    });
  };
  const handleCancel = () => {
