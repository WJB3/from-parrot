...
export default function UserServiceList(props: GT.Modal.Props) {
  ...
  const { data: treeData } = useRequest(() =>
    api.service
      .getDepartment({ params: { current: -1, size: -1 } })
      .then((res) => {
        return res.list;
      })
  );
  ...
  const onReset = (record: GT.Model.UserTeacher) => {
    Modal.confirm({
      ...
      onOk: () => {
        api.user.resetOtherPassword(record.userId).then(() => {
  ...
  const onForbidden = (record: GT.Model.UserTeacher) => {
    Modal.confirm({
      ...
      onOk: () => {
        api.user.forbiddenOther(record.userId).then(() => {
  ...
  const updateRole = (record: GT.Model.UserTeacher) => {
    api.user.getOtherRoles(record.userId).then((roleIds) => {
  ...
  const onResume = (record: GT.Model.UserTeacher) => {
    Modal.confirm({
      ...
      onOk: () => {
        api.user.resumeOther(record.userId).then(() => {
