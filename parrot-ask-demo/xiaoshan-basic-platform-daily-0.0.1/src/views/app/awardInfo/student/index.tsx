...
// 学生获奖情况登记列表
export default function StudentAwardListPage(props: RouteComponentProps) {
  const [form] = Form.useForm();
  const modal = useRef<GT.Modal.Ref>();
  const [taskId, setTaskId] = useState<number>();

  // 任务类型 学生竞赛
  const taskType = { type: 1 };
  ...
  const editTaskDetail = (record: GT.Model.AwardTaskModel) => {
    console.log('编辑');
    props.history.push(`${props.location.pathname}/publishnewtask?type=1&id=${record.id}&isEdit=${true}`);
  };
  ...
  const columns: ColumnsType<GT.Model.AwardTaskModel> = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space>
          <PrivateComponent id={378}>
            <Button
              size='small'
              type='link'
              onClick={() => {
                props.history.push(`${props.location.pathname}/awardRecord/${record.id}/${record.year}年度${record.name}-汇总记录`);
              }}>
              汇总
            </Button>
          </PrivateComponent>
          <PrivateComponent id={379}>
            <Button
              size='small'
              type='link'
              onClick={() => {
                props.history.push(`${props.location.pathname}/awardPoint/${record.id}/${record.year}年度${record.name}-得分考评`);
              }}>
              得分
            </Button>
          </PrivateComponent>
  ...
  return (
    <div>
      ...
      <PrivateComponent id={[376, 377]}>
        <p>
          <Space>
            <PrivateComponent id={376}>
              <Button
                type='primary'
                ghost
                onClick={() => {
                  props.history.push(`${props.location.pathname}/publishnewtask?type=1`);
                }}>
                发布新任务
              </Button>
            </PrivateComponent>
            <PrivateComponent id={377}>
              <Button
                type='primary'
                ghost
                onClick={() => props.history.push(`${props.location.pathname}/workflow/1`)}>
                审批设置
              </Button>
            </PrivateComponent>
          </Space>
