...
export default function FaceServicePage() {
  ...
  const getTableData = ({ current, pageSize: size }: PaginatedParams[0], formData: any) => {
    ...
    return api.service.getFacePage({
      ...formData,
  ...
  const onUpload = (record: GT.Model.TeacherFace) => {
    api.service.getFaceStatus().then((status) => {
      if (status === 0) {
  ...
  const onExport = () => {
    ...
    api.service.exportFace({...searchParams, departmentIds: searchParams.departmentIds?.join(',')}).then((res) => {
      download(res);
    });
  };
  ...
  const onDelete = (records: GT.Model.TeacherFace[]) => {
    Modal.confirm({
      ...
      onOk: () => {
        api.service.deleteFaces(records.map((record) => record.id)).then(() => {
  ...
  return (
    <div>
      <Form form={form} className='search_form'>
        <Row>
          <Row className='search_form_items'>
            ...
            <Form.Item name='departmentIds' label='服务部门' style={{ width: 250 }}>
              <TreeSelect
                showSearch
                maxTagCount={1}
      ...
      <PrivateComponent id={[300, 301, 302, 303]}>
        <p>
          <Space>
            <PrivateComponent id={300}>
              <Button type='primary' ghost onClick={onBatchImport}>
                批量导入照片
              </Button>
            </PrivateComponent>
            <PrivateComponent id={301}>
              <Button
                type='primary'
                ghost
                onClick={() => {
            ...
            <PrivateComponent id={302}>
              <Button type='primary' ghost onClick={onExport}>
                数据导出
              </Button>
            </PrivateComponent>
            <PrivateComponent id={303}>
      ...
      <UploadAvatarModal
        ...
        onOk={refresh}
        type='service'></UploadAvatarModal>
      <BatchUploadAvatarModal
        onRef={(ref) => (batchModal.current = ref)}
        onOk={refresh}
        type='service'></BatchUploadAvatarModal>
      <UploadRecordsModal onRef={(ref) => (checkModal.current = ref)}
      type='service'></UploadRecordsModal>
    </div>
  );
}
