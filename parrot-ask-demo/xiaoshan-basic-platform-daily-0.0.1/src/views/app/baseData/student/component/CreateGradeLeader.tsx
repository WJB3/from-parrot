...
export default function CreateGradeLeaderModal(
  props: GT.Modal.Props & { sectionId: number; enrollmentYear: number },
) {
  const [visible, setVisible] = useState(false);
  const [title, setTitle] = useState('设置年级组');
  ...
  return (
    <Modal
      ...
      <Form preserve={false} form={form} initialValues={{}} {...constant.form.layout}>
        ...
        <Form.Item name='teacherId' label='年级组领导' rules={[{ required: true }]} hasFeedback>
          <Select
            showSearch
            showArrow
            onSearch={onSearch}
