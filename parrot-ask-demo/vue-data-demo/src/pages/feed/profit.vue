<template>
  <div class="home">
    <Tabbar title="累计收益" :leftArrow="true" />
    <!-- rightText="转出" -->
    <div class="header1">
      <div style="color:#FBA318">{{add_income}}</div>
      <div>累计收益</div>
    </div>
    ...
    <!-- <van-tabs v-model="active" background="#FFF9E7" swipe-threshold="6" >
...
<script>
// import Vue from "vue";
import { inBillList, userAccount } from "@/api/home";
...
export default {
  ...
  created() {
    userAccount().then(res => {
      this.add_income = res.add_income;
    });
    inBillList({ page: 1, pagesize: 10000 }).then(res => {
      this.arr = res;
    });
  },
  data() {
