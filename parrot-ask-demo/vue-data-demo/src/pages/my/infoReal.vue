<template>
  <div class="home">
    ...
    <p class="tips">*完善信息确认通过后不可修改，请填写真实信息。</p>
    <van-button class="submit" :disabled="disabled" @click="submit()">{{audioTip}}</van-button>
  </div>
</template>
...
export default {
  ...
  created() {
    userInfo().then(res => {
      this.temp = res;
      console.log(res.examine);
      if (res.examine == 2 || res.examine == 3) {
        this.disabled = false;
      } else {
        this.disabled = true;
      }
      ...
      if (res.examine == 0) {
        this.audioTip = "认证中";
      } else if (res.examine == 1) {
        this.audioTip = "认证成功";
      } else if (res.examine == 2) {
        this.audioTip = "已拒绝,重新提交";
      } else if (res.examine == 3) {
        this.audioTip = "确定";
      }
    });
  },
