...
function drawLeaflet(_self) {
  ...
  map.on("popupopen", function() {
    ...
    leafletMonitorButton.addEventListener(
      "click",
      function() {
  ...
  d3.queue()
    ...
    .await(function(error, siteData, spotData, shopData) {
      ...
      iconSet["site"] = new LeafIcon({
        iconUrl: _self.getStaticAssetPath + "dead.png",
      });
      iconSet["shop"] = new LeafIcon({
        iconUrl: _self.getStaticAssetPath + "hurt.png",
      });
      iconSet["spot"] = new LeafIcon({
        iconUrl: _self.getStaticAssetPath + "damage.png",
      });
      ...
      function defineMarker(dataType, d, cluster, markerIcon) {
        ...
        var monitorIcon =
          '<input type="button" value="启动监控" id="leafletMonitor">';
        ...
        var title =
          ...
          "<br>" +
          "编号：" +
          (d.Site_id || d.Shop_id || d.Spot_id) +
          "<br>" +
          monitorIcon;

        var marker = L.marker(new L.LatLng(+d.Lat, +d.Lng), {
          title: title,
          icon: markerIcon,
        });
